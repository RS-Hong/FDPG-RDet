...
class LFDCBlock(nn.Module):
    ...
    def _build(self, C, device, dtype):
        # 计算处理/旁路通道，保证有旁路
        c_proc = max(1, round(C * self.ratio))
        if C > 1:
        ...
        if c_proc >= 3:
            p1 = max(1, round(c_proc * 0.25))
            p2 = max(1, round(c_proc * 0.50))
            p3 = c_proc - p1 - p2
            if p3 < 1:
                need = 1 - p3
                take = min(need, max(0, p2 - 1))
                p2 -= take
                need -= take
                if need > 0:
                    take = min(need, max(0, p1 - 1))
                    p1 -= take
                    need -= take
                p3 = 1
        elif c_proc == 2:
        ...
        self.se = nn.Sequential(
            nn.Linear(C, C // r, bias=False), nn.ReLU(True), nn.Linear(C // r, C, bias=False), nn.Sigmoid()
        ).to(device, dtype)
    ...
    def forward(self, x):
        ...
        idx = 0
        if p1 > 0:
            o1 = self.f1(parts[idx]) * beta[:, 0:1]
            outs.append(o1)
            idx += 1
        if p2 > 0:
            o2 = self.f2(parts[idx]) * beta[:, 1:2]
            outs.append(o2)
            idx += 1
        if p3 > 0:
            o3 = self.f3(parts[idx]) * beta[:, 2:3]
            outs.append(o3)
            idx += 1
...
class C2f_LFDC(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, ratio=0.25):
        ...
        if isinstance(n, (list, tuple, bool)):
            n = int(n)
        if isinstance(shortcut, (list, tuple)):
            shortcut = bool(shortcut[0])
