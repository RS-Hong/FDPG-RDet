...
"""
# --------------------------------------------
# Improved implementation of Converse2d operator ^_^
# --------------------------------------------
"""
...
class Converse2D(nn.Module):
    ...
    def forward(self, x):
        ...
        FBFy = FBC.float() * torch.fft.fftn(STy.float(), dim=(-2, -1))

        FR = FBFy.float() + torch.fft.fftn((biaseps * x).float(), dim=(-2, -1))
        x1 = FB.mul(FR.to(x.dtype))
    ...
    def splits(self, a, scale):
        ...
        *leading_dims, W, H = a.size()
        ...
        # 保持原始的reshape和permute逻辑，确保功能一致性
